...
export async function callCurationLLM(options: LLMRequestOptions): Promise<string> {
  ...
  switch (CURATION_LLM_PROVIDER) {
    ...
    case 'anthropic':
      return fetchAnthropic(ANTHROPIC_API_KEY, model, options);
    case 'ollama':
      return fetchOllama(OLLAMA_URL, model, options);
    default:
      throw new Error(`Unknown curation LLM provider: ${CURATION_LLM_PROVIDER}`);
  }
}
...
async function fetchOpenAICompatible(
  ...
  if (!apiKey) throw new Error(`Missing API key for ${CURATION_LLM_PROVIDER}`);
...
async function fetchAnthropic(
  ...
  if (!apiKey) throw new Error('Missing ANTHROPIC_API_KEY for curation');

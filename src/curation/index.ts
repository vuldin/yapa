...
export async function curationCycle(): Promise<CurationStats | null> {
  ...
  try {
    ...
    for (const c of collections) {
      ...
      for (let i = 0; i < unclassified.length; i += CURATION_BATCH_SIZE) {
        const batch = unclassified.slice(i, i + CURATION_BATCH_SIZE);
        try {
 ...
 * then schedules recurring cycles every CURATION_INTERVAL_MS.
 */
export async function startCuration(): Promise<void> {
  ...
  process.stderr.write(
    `[yapa-curation] Background curation started (interval: ${CURATION_INTERVAL_MS / 1000}s)\n`,
  );
}

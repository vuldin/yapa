...
import { ARTIFACTS_DIR, TRAINING_BACKEND, TRAINING_BASE_MODEL } from '../config.js';
...
function getBackend(): TrainingBackend {
  switch (TRAINING_BACKEND) {
    case 'fireworks':
      return new FireworksBackend();
    default:
      throw new Error(`Unknown training backend: ${TRAINING_BACKEND}`);
  }
}

function previewDir(): string {
  const dir = join(ARTIFACTS_DIR, 'training-runs');

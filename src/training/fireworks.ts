...
import { TRAINING_FIRECTL_PATH } from '../config.js';
import type {
  AdapterArtifact,
  TrainingBackend,
  TrainingJobConfig,
...
const defaultRunner: FirectlRunner = {
  async run(args) {
    return new Promise<FirectlResult>((resolve, reject) => {
      const child = spawn(TRAINING_FIRECTL_PATH, args, { stdio: ['ignore', 'pipe', 'pipe'] });
      let stdout = '';
      let stderr = '';
      ...
      child.on('error', err => {
        if ((err as NodeJS.ErrnoException).code === 'ENOENT') {
          reject(new Error(
            `firectl not found at "${TRAINING_FIRECTL_PATH}". Install it from https://docs.fireworks.ai/tools-sdks/firectl or set YAPA_TRAINING_FIRECTL_PATH to its location.`,
          ));
        } else {
          reject(err);
        }

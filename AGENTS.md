...
7. Write the YAPA behavioral instructions. Append the following block to the correct file based on the user's SCOPE and editor choice from Phase 2:
   ...
   **Do not store:** ephemeral conversation state, obvious code patterns, anything
   ...
   - Before the session ends, call `journal_consolidate` to merge the drafts into
     a single memory tagged `journal`. The next session's recall will surface it.
   ...
   ### Periodic Compaction
   - When `task_list` surfaces a "compaction candidate" task (auto-created when a
     collection crosses the size threshold), call `compaction_suggest` for the
     named collection, write a rolling summary of each group, and submit it via
     `compaction_apply`. Originals get archived (filtered from recall by default).

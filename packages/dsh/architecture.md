...
| Capturing findings from agent responses | `session/event` → `user/message` / `assistant/message` / `turn/end` | Per-session turn buffer judged once per turn by the aux-LLM **extractor** (`curation/extractor.ts`, versioned prompt). Heuristic prefilter (min chars, skip blocked turns) gates the LLM call. Candidates then go through **retrieve-then-decide**: near neighbors are presented to the conservative **resolver** (`curation/resolver.ts`) which picks skip / add / supersede — a changed fact *replaces* the stale memory (old one archived with `superseded_by`, recoverable) instead of being lost to dedup; a mid-turn agent-side `yapa_memory_store` still dedupes to skip. Auto-captured memories are clamped (salience ≤ `captureMaxSalience`, default 2.0), tagged `auto-capture`, and carry provenance metadata (`session_id`, `turn`, prompt versions). The next injection surfaces a one-line capture notice. Fully async and fail-open — extraction never blocks the agent loop. |
| Contradiction hygiene over time | `ctx.interval` (daily) + `yapa_janitor_now` tool | The **janitor** (`curation/janitor.ts`) scans for near-duplicate pairs already in the store (agent-side stores bypass the capture gate) and runs the same conservative resolver: same-fact duplicates archived (`duplicate_of`), stale memories superseded by merged updates, distinct facts kept. Bounded per run (`janitorMaxPairs`, default 20), fail-open per pair. Same reversible archive contract as memory compaction (`archived: true`, filtered from recall/list by default). |

...
### Removal
To remove YAPA from this harness, the user runs \`dsh plugin --profile <name> remove @yapa/dsh-plugin\` and deletes the plugin's row from the profile's \`cordis.patch.yml\`. Collections (memories/tasks) are preserved unless the user explicitly deletes them.
`;
